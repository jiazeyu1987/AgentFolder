        ...
        if total_score >= int(cfg.plan_review_pass_score):
            ...
            try:
                ...
                stage_gen_id = record_llm_call(
                    ...
                    scope="PLAN_GEN",
                    provider="SYSTEM",
                    prompt_text="AUTO(BINDINGS): no-op (upstream artifacts injected at run-time based on DEPENDS_ON)",
                    response_text="AUTO(BINDINGS): no bindings written; runtime prompt injection uses DEPENDS_ON + approved/active artifacts",
                    started_at_ts=now,
                    finished_at_ts=now,
