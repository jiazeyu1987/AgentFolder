...
def test_reset_db_reports_failure_when_db_is_write_locked():
    # If another process holds a write lock, reset cannot safely wipe tables.
    with tempfile.TemporaryDirectory() as td:
        td_path = Path(td)
        db_path = td_path / "state.db"
        conn = connect(db_path)
        ...
        assert res.get("mode") in {"wipe_failed", "failed"}
